    ...
    for name, m in quantized_model.named_modules():
        if isinstance(m, (Quantized_Conv2d, Quantized_Linear)):
            if name != first_conv_name:
                ...
                act = input_activation.get(name, None)
                if act is None:
                    act = output_activation.get(name, None)
                if act is None:
                    warnings.warn(
                        f"Skipping activation calibration for layer '{name}' because no activation was captured. "
                        "The layer will keep its default scale values."
                    )
                    continue
