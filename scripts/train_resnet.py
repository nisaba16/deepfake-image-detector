"""Training script for ResNet50 deepfake detection.

This script trains a 6-channel ResNet-50 model:
1. Uses SixChannelPreprocessor to generate 6-channel inputs
2. Trains DeepfakeResNet50 end-to-end with PyTorch
...
from models.custom_resnet import DeepfakeResNet50, export_to_onnx
...
def train_model(
    ...
    """Train the ResNet-50 model with learning rate scheduling.
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train ResNet-50 for deepfake detection')
    ...
    parser.add_argument('--freeze-layers', type=str, default='layer2', choices=['none', 'conv1', 'layer1', 'layer2', 'layer3'],
                        help='Freeze early layers to prevent overfitting (default: layer2)')
...
def main():
    ...
    # Freeze early layers if requested (prevents overfitting on small datasets)
    if args.freeze_layers != 'none':
        model.freeze_early_layers(freeze_until=args.freeze_layers)
    ...
    # Export to ONNX
    if args.export_onnx:
        onnx_path = output_dir / 'deepfake_resnet50.onnx'
